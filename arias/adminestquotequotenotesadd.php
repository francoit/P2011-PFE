...
<?php //adminestquotequotenotesadd.php
     echo texttitle('Quote Notes Add');
     if ($quotebold) { //if the user has submitted info
//            checkpermissions('est');
             if ($conn->Execute('insert into estquotequotenotes (quotebold,quotetext,showwhen,orderflag,gencompanyid,entrydate,entryuserid,lastchangeuserid) VALUES ('.sqlprep($quotebold).', '.sqlprep($quotetext).', '.sqlprep($showwhen).', '.sqlprep($orderflag).', '.sqlprep($active_company).', NOW(), '.sqlprep($userid).', '.sqlprep($userid).')') === false) {
                echo texterror("Error adding notes for quote.");
             } else {
                echo textsuccess("Quote Notes added successfully.");
             };
     };
     echo '<form action="adminestquotequotenotesadd.php" method="post" name="mainform"><input type="hidden" name="nonprintable" value="1"><table>';
     ...
     echo '<tr><td align="'.TABLE_LEFT_SIDE_ALIGN.'">Show on quote when:</td><td><select name="showwhen"'.INC_TEXTBOX.'>';
          echo '<option value="'.SD_SHOW_ALWAYS.'">Always Show';
          echo '<option value="'.SD_SHOW_ONE_INK.'">One Ink Only';
          echo '<option value="'.SD_SHOW_MT_ONE_INK.'">More than One Ink';
     echo '</select></td></tr>';
