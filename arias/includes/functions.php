...
<?
	...
	function check_system()
	{
		...
		{
			?>
				<style>body {background-color:#445566} td {text-align:center; vertical-align:middle;}</style>
				<table style="background-color:#ddddda; color:red;" width="100%" height="100%" cellpadding="20">
				<tr><td>&nbsp;</td></tr>
				<tr><td height="40" style="font-size:14pt">PHP configuration not yet done.</td></tr>
				<tr><td height="40" style="font-size:12pt">In order to use ARIA, you need to edit PHP's configuration file named PHP.INI. Depending on your platform, it may be in different places. In Windows, for example, it is most probably in C:\Windows.</td></tr>
				<tr><td height="40" style="font-size:14pt">In PHP.INI, you need to set <B>register_globals</B> to <B>ON</B>.</td></tr>
				<tr><td>&nbsp;</td></tr>
				</table>
			<?
			die();
		...
		$test_conn->debug=0;
		$success = $test_conn->PConnect(DB_SERVER, DB_SERVER_USERNAME, DB_SERVER_PASSWORD, DB_DATABASE);
		if (!$success)
		{
			?>
				<style>body {background-color:#445566} td {text-align:center; vertical-align:middle;}</style>
				<table style="background-color:#ddddda; color:red;" width="100%" height="100%" cellpadding="20">
				<tr><td>&nbsp;</td></tr>
				<tr><td height="40" style="font-size:12pt">Could not connect to the database. <BR><BR>Error message = <?= $test_conn->ErrorMsg()?><BR><BR>If ARIA is not installed yet, point your browser to this: <A HREF="setup/">Install</A></td></tr>
				<tr><td>&nbsp;</td></tr>
				</table>
			<?
			die();
		...
		if (DB_SERVER_PASSWORD == "changethis")
		{
			?>
				<style>body {background-color:#445566} td {text-align:center; vertical-align:middle;}</style>
				<table style="background-color:#ddddda; color:red;" width="100%" height="100%" cellpadding="20">
				<tr><td>&nbsp;</td></tr>
				<tr><td height="40" style="font-size:14pt">Change the password for database. It is defined in includes/my_defines.php.</td></tr>
				<tr><td>&nbsp;</td></tr>
				</table>
			<?
			die();
		}
/*		if (ENCRYPTION_KEY == "8aw3wb7v35n9awv48b7o8c7a4bd")
		{
			?>
				<style>body {background-color:#445566} td {text-align:center; vertical-align:middle;}</style>
				<table style="background-color:#ddddda; color:red;" width="100%" height="100%" cellpadding="20">
				<tr><td>&nbsp;</td></tr>
				<tr><td height="40" style="font-size:14pt">Change the encryption key. It is defined in includes/my_defines.php</td></tr>
				<tr><td>&nbsp;</td></tr>
				</table>
			<?
			die();
...
*/
     ...
     function checkdec($number, $decimalplaces) {
          //if ($number==bcadd(0,$number,$decimalplaces))
              $number=bcadd(0,$number,$decimalplaces);
          return $number;
     };
     ...
     function dynbuttonrollover($href, $text, $size = '10', $font = MENU_FONT, $transparent1 = 1, $transparent2 = 1, $border = 0, $bg1 = COLOR_BG1, $bg2 = COLOR_BG2, $t1 = MENU_COLOR1, $t2 = MENU_COLOR2) {
         //generate dynamic image
         if (strtoupper($href)=="INDEX.PHP?LOGOUT=1") {
             $target="_parent";
         } else {
             $target="main";
         };
     ...
     function navlinks() {
         global $lang, $docfilename, $conn;
         $recordSet=&$conn->Execute('select menucategory.id, menucategory.name, menufunction.link, menufunction.name, menupage.menufunctionid from menupage left join menufunction on menupage.menufunctionid=menufunction.id left join menucategory on menufunction.menucategoryid=menucategory.id where menupage.name='.sqlprep($docfilename).' order by menucategory.id desc');
         if ($recordSet&&!$recordSet->EOF) {
               $menuSTR='STR_MENU_'.$recordSet->fields[0];
               $submenuSTR='STR_SUBMENU_'.$recordSet->fields[0].'_'.$recordSet->fields[4];
               $foo='<DIV class="navlinks"><a class="navlinks" href="start.php">'.$lang['STR_HOME'].'</a> > <a class="navlinks" href="explain.php?menucategoryid='.$recordSet->fields[0].'">'.rtrim($lang[$menuSTR]).'</a> > ';
               if ($recordSet->fields[2]<>$docfilename) {
                   $foo.='<a class="navlinks" href="'.rtrim($recordSet->fields[2]).'">'.rtrim($lang[$submenuSTR]).'</a>';
               } else {
                   $foo.='<a class="navlinks" href="'.rtrim($docfilename).'">'.rtrim($lang[$submenuSTR]).'</a>';
               };
               $foo.='</DIV>';
               return $foo;
         };
