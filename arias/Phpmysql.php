...
function logon() {
    ...
    echo "<tr><td>Hostname </td><td> <input type=text name=hostname></td></tr>\n";
...
function listDatabases() {
    ...
    for( $i = 0; $i < $num; $i++ ) {
        ...
        echo "<tr>\n";
        echo "<td>$dbname</td>\n";
        echo "<td><a href='$PHP_SELF?action=listCreateTables&dbname=$dbname'> Show Create </a></td>\n";
...
/*----------------------------------------------------------------
 * This will dump the tables and create insert statements for data
 */
function listCreateTables() {
    global $mysqlHandle, $dbname, $PHP_SELF;
/*
 * Create an array to handle this
 */
	$tablelist = array();
/*
...
/*
 * Get the count of tables, create an array of table names
 */
...
/*
 * select the database for some selects
 */
 ...
 */
	for ($i = 0; $i < count($tablelist); $i++) {
    	$rs = mysql_query( "show create table " . $tablelist[$i] );

        $field = mysql_fetch_array( $rs );

        $fieldname = $field["Create Table"];

		?> <pre> <?
		echo "drop table " . $tablelist[$i] . ";\n";
		echo $fieldname . ";\n";
/*
 * Now dump the data as a series of inserts
 */
		showInserts($tablelist[$i],100);
		?> </pre> <?
	}
}
/*-------------------------------------------------
 * This function will dump table data up to a limit
 */
function showInserts($tablename,$limit)
{
 	global $mysqlHandle, $dbname, $PHP_SELF;
...
/*
 * Get the count of fields and the count of rows
 */
...
if( $action == "logon" || $action == "" || $action == "logout" )
    ...
    if( $mysqlHandle == false ) {
        ...
        echo "<body>\n";
        echo "<table width=100% height=100%><tr><td><center>\n";
        echo "<h1>Error: Connect failed: ".mysql_error()."</h1>\n";
...
/*
 * Handle dump first so we can have a clean output
 */
		if( $action == "listCreateTables" ) {
            listCreateTables();
